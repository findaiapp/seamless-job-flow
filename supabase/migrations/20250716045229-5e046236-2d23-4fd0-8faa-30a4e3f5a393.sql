...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'applied_jobs' AND policyname = 'Users can view their own applied jobs'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'applied_jobs' AND policyname = 'Users can create their own applied jobs'
  ) THEN
